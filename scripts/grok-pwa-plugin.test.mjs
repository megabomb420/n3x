...
// The checkout is itself a published N3X app: src/lib/og/site.json and
// public/og.jpg are real, and any ctx that omits `cwd` reaches them through
// process.cwd() — pinning og:title to the site's own title and og:image to its
// own card. Point every fixture at an empty directory instead; tests that need
// a site or a card pass their own cwd/site explicitly.
const FIXTURE_ROOT = mkdtempSync(join(tmpdir(), "grok-pwa-fixture-"));
const inject = (html, ctx = {}) => injectGrokPwaHead(html, { cwd: FIXTURE_ROOT, ...ctx });
...
test("streaming injector matches </HEAD> case-insensitively", () => {
  const injector = createHeadInjector({ appName: "Wild Race", cwd: FIXTURE_ROOT });
  const chunks = [
...
test("streaming injector passes post-head chunks through untouched", () => {
  const injector = createHeadInjector({ cwd: FIXTURE_ROOT });
