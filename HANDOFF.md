...
Stack: TanStack Start, React 19, Tailwind v4, Vite, Nitro preset `vercel`. App shell is a locked full-height column: `html, body { height: var(--app-h, 100%) }` and the shell is `h-full`, so header and nav do not scroll, only `<main>` does. Do not switch to `position: fixed` or document scroll without migrating the pull-to-refresh and per-tab scrolling contract. The earlier claim that `viewport-fit=cover` was required was not proven: the current trial uses WebKit's automatic safe-area insetting. `.safe-top`/`.safe-bottom` apply only in `display-mode: standalone`, avoiding double padding in ordinary webviews.

**Measured on an iPhone 17 Pro / iOS 27 installed app with `viewport-fit=cover`** (`?diag=1`, one tap from the header pill → About): `inner`/`vv`/`doc` **812**, screen and `100lvh` **874**, `svh`/`dvh` **812**, top inset **62px**, bottom inset **34px**, `standalone true`. Its nav ends at 812, and a fixed bar at the bottom of `100lvh` does not show. A pinch-out temporarily lets the shell reach 840 via `--app-h`; at rest, forcing 840 with unchanged viewport clips the nav. `AppShell` sets `--inset-bottom: 0` only when the viewport is short and the top inset is real, so it does not reserve a home-indicator inset outside the short view.
...
### OPEN: the dead band under the nav (iOS 27 standalone)

**Not solved for this N3X installation.** These numbers were measured on the owner's iPhone 17 Pro running iOS 27, installed app, through `?diag=1`. AnimeBuddy on the same device is reported to position its bar lower; compare its chrome configuration before ruling out a platform-mode difference.
...
**At rest in that installation, the viewport stops at 812.** A `position: fixed` bar at the bottom of `100lvh` is invisible there. During pinch-out the visual viewport temporarily grows and the nav can reach 840; that does not prove the same area remains available after the gesture ends. The current trial asks WebKit for the automatic safe-area rectangle instead of `viewport-fit=cover`.
...
- `black-translucent` → `black` plus `color-scheme: dark` (834b08d), then `default`, then a retry of AnimeBuddy's `black-translucent`: the owner reports no improvement; status-bar variants alone have not expanded N3X's viewport. The new experiment removes `viewport-fit=cover` while using `black` for the opaque status bar.

**Current experiment.** With no `viewport-fit=cover`, WebKit's default `auto` viewport should inset the page into the safe rectangle; the header's `env top` is expected to fall when the status bar is already outside the page. Measure `inner`, `vv`, `doc`, insets and nav again in the installed app. Do not assume a successful build or Chromium screenshot proves this iOS-specific behavior. Leave the probe in place; export device-local data before any icon reinstall.
...
4. **The bottom viewport trial:** opaque and translucent status-bar modes did not resolve the band with `viewport-fit=cover`. Automatic safe-area insetting is under test; do not report it as fixed without an installed-app screenshot showing the nav lower with all labels visible.
