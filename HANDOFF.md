...
Stack: TanStack Start, React 19, Tailwind v4, Vite, Nitro preset `vercel`. App shell is a locked full-height column: `html, body { height: var(--app-h, 100%) }` and the shell is `h-full`, so header and nav do not scroll, only `<main>` does. Do not switch to `position: fixed` or document scroll. `viewport-fit=cover` is required: `.safe-top`/`.safe-bottom` pad with `env(safe-area-inset-*)` (plus 1.1 rem at the top so the header's bright mark and title stay out of iOS 26's top edge blur, which otherwise smears a ghost of them into the status bar), and they only apply in `display-mode: standalone` so the Grok/Safari webviews, which already sit below the notch, are not double-padded.

**Why `--app-h` exists.** Every viewport unit is wrong on the installed app in one direction or another: on iOS 26 standalone the layout viewport (`documentElement.clientHeight`, which is also what `height: 100%` and `100svh` resolve to) came back ~96 pt shorter than the screen — the column stopped early and left a dead band under the nav, measured at 106 css px on an iPhone 17 Pro screenshot — while the web view kept painting the full screen. `AppShell` therefore measures the visible viewport (`window.innerHeight` / `visualViewport.height`) and sets `--app-h` only when it is genuinely taller than the layout viewport, and only in standalone, so Safari's toolbars can never push the nav off screen. `?diag=1` (`/components/layout-probe.tsx`, one tap from the header pill → About) prints inner/vv/doc/`100lvh`/env insets and outlines the layout viewport in cyan and the app column in magenta — the only way to read those numbers is a screenshot from the installed app.
