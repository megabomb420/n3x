...
On the owner's iPhone 17 Pro running iOS 27, the installed N3X probe reports `inner`/`vv`/`doc` 812, screen 874, `env top` 62, bottom 34, and `nav 759→812`: the 62px band lies outside that installation's viewport. The owner reports that AnimeBuddy on the **same phone** places its bottom bar lower. AnimeBuddy uses `apple-mobile-web-app-status-bar-style: black-translucent` and `position: fixed; bottom: 0`, whereas N3X used `default` and a flex footer. In Chromium with both pages given a viewport of 812px, **both navs end at 812px**; fixed positioning alone does not explain the phone difference. N3X is trying AnimeBuddy's `black-translucent` chrome setting, without changing the nav layout. This is a device experiment, not a confirmed fix; WebKit may preserve the status-bar mode from when the Home Screen icon was installed. Pinch zoom remains blocked, and touch-landscape shows a rotate-back prompt rather than a hardware orientation lock.
...
The iOS 27 bottom band is confirmed by the owner's `?diag=1` screenshot; the AnimeBuddy comparison above supersedes the earlier inference that no installed web app could put its nav lower. The new `black-translucent` trial still needs an installed-iPhone measurement. The owner will unpublish `n3x.grok.me` personally; this work does not touch that host. A push to `main` automatically updates GitHub Pages; Cloudflare Pages requires its own deploy.
...
- `black-translucent` → `black` plus `color-scheme: dark` (834b08d): intended to put the web view below the status bar; the owner's normal screenshot still showed a band. A `black` → `default` trial also left `inner`/`vv`/`doc` at 812 on iOS 27. AnimeBuddy uses `black-translucent`; N3X now tries that setting again with the current iOS installation.

**Current experiment.** Match AnimeBuddy's `apple-mobile-web-app-status-bar-style: black-translucent`, then compare N3X's installed-app probe with its previous `nav 759→812`, `screen 874` result. Do not assume that changing the hosted HTML changes an existing Home Screen icon's saved chrome. If reinstalling the icon is needed, export device-local data first and warn that the result is uncertain. Leave the probe in place for the measurement.
...
4. **The status-bar variants:** `default` was measured on iOS 27 and did not remove the band; N3X is retrying AnimeBuddy's `black-translucent` mode. That mode is deprecated and the earlier N3X trial did not resolve the issue, so do not report this as a fix until a new installed-app probe shows a larger viewport.
