...
- https://n3x-api.whip-blanket.workers.dev — the backend Worker; `/health` says whether its API key is configured. Surface: `GET /club`, `/player/<tag>`, `/battles/<tag>`, `/ladder?type=players|clubs&country=…`, `/maps`, `/map-stats?map=…&mode=…`, `/tier-list?scope=overall|ranked`, `/creators`, `/creators/<id>`, and the guarded `/__warm?index=N`.

Five tabs plus **Settings**: **Club**, **Stats**, **Meta**, **Ladder**, **Maps**, and a settings screen (language English/Polski using the game's own Polish names, export/import/reset of this device's saved data, and the build version against `version.json` so an old shell can tell it is behind). Club, player, ladder, rotation and one map's own picture come from the official Brawl Stars API and the `map-stats` endpoint through the Worker; the tier list and a map's per-brawler numbers are parsed from BrawlMetrics' public tables and re-read about every 15 minutes; creator links come from public YouTube feeds. Nothing reads Brawl Time Ninja any more.
...
**The installed iOS viewport question is closed (23 Sep 2026, owner-confirmed).** On the owner's iPhone 17 Pro / iOS 27 the home-screen icon was deleted and re-added; that refreshed installation reaches the screen bottom, which ended the dead band. The old `viewport-fit=cover` icon had reported `inner`/`vv`/`doc` 812 against an 874 screen, `env top` 62, bottom 34 and `nav 759→812`, with a pinch-out screenshot showing the same nav at 787→840 while the header stayed put — `--app-h` tracked a temporarily taller visual viewport, and freezing it at 840 after the gesture clipped the labels. The refreshed app then put the tab labels under the gesture indicator, because WebKit reported a **zero** bottom inset, so `.safe-bottom` keeps the native inset and floors it at 34px on an installed iOS phone (the old `--inset-bottom: 0` exception is gone). The owner confirms the tabs now clear the indicator. Pinch zoom remains blocked; touch-landscape shows a rotate-back prompt, not a hardware orientation lock. `?diag=1` still reports `inner`/`vv`/`doc`/`lvh`, both native insets and `nav-pad` should this class of problem return.
...
The iOS 27 bottom band was confirmed by the owner's `?diag=1` screenshot. The AnimeBuddy comparison and the N3X pinch-out screenshot invalidate the earlier blanket claim that the bar can never be lower; the viewport *does* temporarily expand during a pinch, while retaining that CSS height after it shrinks clips the labels. The `black-translucent` trial made no difference according to the owner. The automatic safe-area viewport experiment above needs a fresh installed-app screenshot; it is not yet a fix. The owner will unpublish `n3x.grok.me` personally; this work does not touch that host. A push to `main` automatically updates GitHub Pages; Cloudflare Pages requires its own deploy.

## Parity check against Brawl Time Ninja and Brawlify (23 Sep 2026)

Asked whether this app falls behind those two sites. Measured, not assumed — every claim below comes from a real browser or a real HTTP response from this machine, on this date.

**What they have, and why it cannot be copied.** Brawl Time Ninja's map pages are global tier lists: Beach Ball (Brawl Ball) reads 2.72M battles sampled, ±0.53% margin, an "Adjusted Win Rate" column beside "Use Rate", a trophy-range filter and a best-teams table; its brawler, player and club pages come from the same Cube aggregation (a player page leads with hours-played estimates, an account rating against 538,269 players, and progression to a fully maxed account). Brawlify shows per-live-event `BEST PICKS` win-rate deltas tailored to a trophy range plus `MOST USED`, a 2-week trophy trend, an activity heatmap, 28-day tracking tiles and per-mode W/L with trophy deltas. None of it is reachable from a hosted app: `/`, `/maps`, `/brawlers`, `/stats` and `/clubs` on brawlify.com answer **403** to every path even from a residential IP with a browser User-Agent (their API, `api.brawlapi.com`, is open and is where this app's catalog already comes from); Brawl Time Ninja is challenged from datacenter IPs; and BrawlAPI's `/v1/events` answers `{"active":[],"upcoming":[]}`, so the public catalog carries no per-event picks at all.

**What was checked in a browser, asset by asset.** `cdn.brawlify.com` map art (690–1860 px) and brawler portraits load from an app page, and so do `media.brawltime.ninja` brawler avatars, `?size=512` map art and player avatars — which means `src/components/portrait.tsx` still carries a live Brawl Time Ninja fallback, and the older claim in this file that "the client no longer knows Brawl Time Ninja exists" is wrong about that one line. BrawlAPI's `star-powers/…` and `gadgets/…` image URLs answer **404**, and `cdn-misc.brawlify.com` does not answer at all, so a brawler page built from that catalog can carry names, rarities, classes and descriptions but not those icons.

**Shipped with the check — one map's own screen (`/maps/$map`).** Both sites have per-map pages; this app had none since `/maps/$mode/$map` went with the Cube rewrite, and tapping an event card did nothing. A rotation card is now a link and a Stats map row is a link, and the page shows the map art with a live badge and the event window when the map is in the current rotation, the club's own numbers on it (battles, win rate, wins, members, window) behind the All / Ladder / Ranked tabs, the best brawlers on it, the mode split and the members' recent battles there. Every number comes from the members' own logs and carries its sample size; nothing global is invented. *(Superseded later the same day — see "One map's picture, and the publisher's per-map numbers" below: a card now opens the map's full picture, and the map screen shows the publisher's global table instead of the club's rates and queue tabs.)* The screen shares the queue choice with Stats (`src/lib/club/queue.ts`), the row components moved to `src/components/stat-rows.tsx` so both screens render one shape, and `formatWindow` moved to `src/lib/maps/rotation.ts` where it speaks through `maps.ends` / `maps.starts` / `maps.timeUnknown` — those three keys existed in both dictionaries and were never used, so a Polish interface printed "Ends 16:39" in English. `AppShell title="Member"` is dictionary-backed now too (`member.title`).

**Measured while verifying, and worth knowing.** Every solo/duo Showdown battle in the live payload has `result: null` and `victory: null` — the API publishes a placement, not a win or a loss — so `aggregateBattles` drops them. A Showdown map's page used to look empty while the rotation said LIVE; since the change below it carries the publisher's placement table (top-4 rate and games), and only our own battles section can still be empty. The club had 51 competitive games on Acid Lakes and 89 on Cavern Churn with no result attached, out of 550 competitive battles in the logs. One line on the map screen now says exactly that instead of leaving the empty state looking broken.

**Still open, in the order they would matter.** Brawler pages (catalog facts plus this club's numbers with that brawler — the shape the map screen now proves, and the entry row already exists in Stats). A player lookup by tag: the Worker's `/player/<tag>` answers and there is still no input for it anywhere in the app. A per-member win-rate summary on the member page, which is what Brawl Time Ninja leads with. Club trophy history from the snapshots the Worker already stores. Per-event best-pick previews on the rotation cards themselves. The picture overlay now carries the publisher's picks for a map; a preview on the card would have to carry an "our club" label to stay honest. And `formatRelative` is English-only, so `common.updated` renders "Zaktualizowano 6 min ago" in Polish in ten call sites — Polish plurals need doing deliberately, not by string concatenation.

## One map's picture, and the publisher's per-map numbers (23 Sep 2026)

Asked to make a map open its **picture**, and to put **global** numbers on the map screens (ideally by trophy range). The picture part is done; the range part is not deliverable from any source this Worker can read, and the screens say so rather than inventing one.

**Where the global numbers come from.** BrawlMetrics again, because it is the only global publisher a hosted Worker can read — the same conclusion the tier list reached. Measured today from this machine: `api.brawlapi.com` has no statistics endpoint (`/v1/tier-list`, `/v1/stats` → 404); `brawlify.com` answers **403** on every path (`/maps`, `/brawlers`, `/stats`, `/clubs`); Brawl Time Ninja is 403 from datacenter addresses. Per map, `https://brawlmetrics.gg/maps/<mode>/<map>` publishes a `Total Battles` figure, a `Last Updated` stamp, four buckets (`Best Picks`, `Winners`, `Most Used`, `Not Recommended`) and a full table.

**No trophy range exists in that source.** `troph`, `league` and `bracket` appear **zero** times on its `/maps` index, on a map page, on `/tier-list`, on `/events` and on `/brawlers/rico`. The only source with a trophy filter is Brawl Time Ninja (`0-3000+ Trophies`), which no hosted build can reach, and this project deliberately runs no machine of the owner's. So the app shows the publisher's **single population with its sample size and its date**, and says in `map.noSplit` that it does not split by range. No range is guessed.

**Two table shapes, and the header decides which.** An ordinary mode's table is `Rank / Class / Brawler / Tier / Win Rate / Use Rate`. A Showdown page — which has no win or loss to count — is `Rank / Class / Brawler / Tier / Games / Avg Rank / 1st Rate / Top 4 Rate`. The parser reads the `<thead>` first and labels its column from it, so a placement rate is never announced as a win rate: the payload carries `metric: "winRate" | "top4"` and the screen prints `Win rate` or `Top 4` plus `Use rate` or `Games` accordingly. Verified against the live pages — Beach Ball (Brawl Ball) `1,334,094` battles, 107 rows, first row Rosa 75% / 0.7%; Acid Lakes (Solo Showdown) `4,389,780`, 107 rows, first row Rosa 78.4% top-4 / 42,735 games.

**Slugs are the publisher's, not ours.** Its index is the only authority (`/maps/brawl-ball/belles-rock` → 404 because the mode is wrong; `/maps/air-hockey/nullscapes` → 404 while `/maps/brawl-hockey/nullscapes` → 200). `GET /map-stats?map=…&mode=…` therefore resolves the name through the publisher's own `/maps` index first (77 cards), prefers the card whose mode agrees, and only then falls back to a slug built from our name. A map the publisher does not list answers **502 `map-stats-unavailable`** — no numbers from thin air — and the map screen shows that honestly. The index parse needed care: a card's inner content runs past 400 characters, so it anchors on `map-card-name` rather than on a short text window.

**What shipped.**

- `worker/index.js`: `parseBrawlMetricsMapPage`, `parseBrawlMetricsIndex`, `brawlMetricsSlug`, `looseKey`, `readBmIndex`, `resolveMapEntry`, `handleMapStats`, routed as `/map-stats`; per-map KV kept a week so a throttled fetch still answers (`stale: true`), index kept six hours, edge cache 900 s, `CACHE_VERSION` untouched because the endpoint is new and no existing payload changed. The cron is deliberately **not** extended to warm 77 map pages.
- `src/lib/maps/map-stats.ts` (`loadMapStats`); `src/components/map-art.tsx` (the fallback chain both screens now share: map art → mode art → the map's initial; `cover` for cards, `contain` so the 690x1050 board is shown whole); `src/components/map-picture.tsx` (the full-screen picture, Escape/backdrop/X to close, the publisher's top picks, its sample size, its date, and the "Everything about this map" link). The sheet is content-sized and centred so the backdrop is a real tap target — a `min-h-full` sheet swallowed it.
- `findMapEvent` in `src/lib/maps/rotation.ts` (+ `rotation.test.ts`): the old screen took the first match from `[...active, ...upcoming]`, so a **next-up** event was badged "Live now". The status now says which of the two it is.
- `src/components/map-screen.tsx`: the club's own rates, queue tabs and member counts are gone; the publisher's table is shown grouped by its **overall** tier — the publisher's own column says that tier is not map-specific, hence the `map.tierNote` caption, with that map's ranking as the order inside a group — followed by our club's own battles as facts.
- Copy: `map.global`, `map.useRate`, `map.games`, `map.top4Rate`, `map.ranksByWinRate`/`Top4`, `map.clubBattles`, `map.sample`, `map.sampleBy`, `map.published`, `map.noSplit`, `map.tierNote`, `map.picture.close`, `map.details`, and `map.note` rewritten around `{source}` with `map.noteClub` for the no-numbers case; `map.club`/`map.recent` deleted; `state.map.body` now reads "Could not read this map's numbers." The Data page records the source, the one-population limit and the Showdown distinction.

**Verified in this session.** `npm run typecheck` clean; `npm test` **208 script tests + 76 TypeScript tests, zero failures** (three new Worker parser tests and two rotation tests included); `npm run build:pages` prerenders 16 pages; `npm run lint` clean on every touched file. Worker deployed (`072d9b61`, edge `https://n3x-api.whip-blanket.workers.dev`) and curled live: Beach Ball `200` with `metric: winRate`, Acid Lakes solo `200` with `metric: top4`, Nullscapes asked as `Air Hockey` resolved through the index to `brawl-hockey/nullscapes` `200`, an unknown name `502 {"error":"map-stats-unavailable"}`. In a real browser at 390x844: a rotation card opens `[role="dialog"]`, the picture reports natural 690x1050 and **displayed ratio 0.657** (uncropped — `object-cover` in the old 2:1 frame measured about 2.0), the strip shows the publisher's three picks with the sample line and "ranked by win rate", and Escape / a backdrop tap / the `aria-label` X each close it while a tap inside the sheet does not; the map screen shows `1.33M` counted by BrawlMetrics with its date, the S+…D groups (107 rows) and our club's battles, with no club rate and no queue tab anywhere; Polish renders "Liczby globalne", "Użycie"/"Gry", "Zamknij zdjęcie mapy", the no-split sentence and the top-4 wording. On the deployed `https://megabomb420.github.io/n3x/maps/` (Pages run `35885694062`, with a service worker and two caches already installed): the overlay opens with the same numbers. Retired map `Skull Creek` shows "Not in the live rotation" with **no** live badge and still gets the publisher's table; an unknown map shows `Map unavailable / The map numbers source did not answer` and no invented numbers.
...
**The roster had no order and no visible rank.** The member list came back in whatever order the club endpoint returned, while the number next to each member pretended to be a rank. It is now sorted by trophies descending (ties by name), so the number means what it says, and the leadership carries its own quiet colour — gold for the president, `ranked` blue for a vice president, `win` green for a senior, each a 3px left edge plus a 7% tint on the row and the role word, with plain members on a transparent edge so the rows stay aligned. Measured: 161,930 → 137,001 down the first eight rows, and `borderLeftColor` `rgb(46,232,255)` for the president, `rgb(91,192,222)` for a vice president, `rgb(110,207,151)` for a senior.
...
Stack: TanStack Start, React 19, Tailwind v4, Vite, Nitro preset `vercel`. App shell is a locked full-height column: `html, body { height: var(--app-h, 100%) }` and the shell is `h-full`, so header and nav do not scroll, only `<main>` does. Do not switch to `position: fixed` or document scroll without migrating the pull-to-refresh and per-tab scrolling contract. The current viewport meta omits `viewport-fit=cover`; the installed icon had to be deleted and re-added for iOS 27 to change its viewport geometry. `.safe-top`/`.safe-bottom` avoid extra padding in ordinary webviews. Installed iOS phone layouts reserve at least 34px below the tabs because WebKit can report a zero bottom inset despite the gesture indicator.

**Historical measurement on an iPhone 17 Pro / iOS 27 icon installed with `viewport-fit=cover`** (`?diag=1`): `inner`/`vv`/`doc` **812**, screen and `100lvh` **874**, `svh`/`dvh` **812**, top inset **62px**, bottom inset **34px**, `standalone true`. Its nav ended at 812, and a fixed bar at the bottom of `100lvh` did not show. A pinch-out temporarily let the shell reach 840 via `--app-h`; at rest, forcing 840 with unchanged viewport clipped the nav. The old `--inset-bottom: 0` exception was removed once the icon was reinstalled.
...
### Historical: the dead band under the nav (iOS 27 standalone)

**Resolved 23 Sep 2026, confirmed by the owner on the phone.** See the status block at the top: deleting and re-adding the Home Screen icon ended the dead band, and the 34px floor on `.safe-bottom` — leaving the device inset otherwise intact — lifted the tabs clear of the gesture indicator, whose overlap the refreshed app had shown. Chromium cannot emulate iOS Home Screen insets, so this came from the device, not from a browser check.

What the older installation gave the page: `inner` = `vv` = `doc` = **812**, screen **874**, `100lvh` **874**, `svh`/`dvh` **812**, `env(safe-area-inset-top)` **62px**, `bottom` **34px**, `standalone true`. Its layout viewport was the screen *minus the status bar*, anchored at the **top**: the probe's `fixed inset-0` outline drew at screen y 0→812, and the header painted under the clock. The last 62 px of the screen had no layout at all.

**At rest in that older installation, the viewport stopped at 812.** A `position: fixed` bar at the bottom of `100lvh` was invisible there. During pinch-out the visual viewport temporarily grew and the nav could reach 840; that did not prove the same area remained available after the gesture ended. The icon reinstall changed this installation-dependent geometry.
...
- The former `--inset-bottom: 0` override while the viewport was short and `env(safe-area-inset-top)` was real (a995f84): it moved the tabs down ~38 px in the old icon but became harmful after reinstall, when the nav reached the gesture indicator. Removed.
- `black-translucent` → `black` plus `color-scheme: dark` (834b08d), then `default`, then a retry of AnimeBuddy's `black-translucent`: the owner reported no improvement; status-bar variants alone did not expand N3X's old viewport. The later change removed `viewport-fit=cover` and used `black` for the opaque status bar.

**Current code.** With no `viewport-fit=cover`, the newly installed icon reaches the bottom. `.safe-bottom` uses the native bottom inset and an iOS-phone 34px floor when that inset incorrectly resolves to zero. `?diag=1` reports `nav-pad` and both native insets for device-side checks. Do not infer actual installed-app geometry from a Chromium screenshot.
...
**Asked for, with historical limitations.**

1. **The dead band under the nav** — done and confirmed on the owner's phone. The original icon's viewport ended at 812; deleting and re-adding the icon fixed that, the refreshed app then overlapped the gesture indicator, and the 34px floor in `.safe-bottom` fixed that.
...
4. **The bottom viewport trial:** opaque and translucent status-bar modes did not resolve the band with `viewport-fit=cover`. The Home Screen icon's installation-time chrome mattered; reinstallation reached the bottom, then the nav needed gesture-indicator clearance.
...
| `src/lib/meta/brawlapi.ts` | Brawler-name index the Meta board counts against, and the map/brawler art lookups |
| `src/lib/maps/rotation.ts` (+ `rotation.test.ts`) | The rotation payload, its window text, and which event a map is on |
| `src/lib/maps/map-stats.ts` | One map's published global numbers (client cache over `/map-stats`) |
| `src/components/map-art.tsx`, `map-picture.tsx` | The shared map picture (cover/contain) and the full-screen picture view |
...
curl -s "https://n3x-api.whip-blanket.workers.dev/map-stats?map=Beach%20Ball&mode=Brawl%20Ball" | head -c 200   # sampleBattles, metric, rows, source BrawlMetrics
curl -s "https://n3x-api.whip-blanket.workers.dev/map-stats?map=Acid%20Lakes&mode=Solo%20Showdown" | head -c 200   # metric is top4: Showdown ranks by placement
```
...
- Maps: a card opens the map's picture whole (its displayed width/height stays its own ratio, about 0.657), with the publisher's picks, its sample size and the note that the publisher has one population; Escape, a backdrop tap and the X each close it. A map's own page shows the global table and our club's battles, and a map the publisher does not list shows the honest error instead of numbers.
