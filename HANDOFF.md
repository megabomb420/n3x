...
On the owner's iPhone 17 Pro / iOS 27, the installed N3X probe with `viewport-fit=cover` reported `inner`/`vv`/`doc` 812, screen 874, `env top` 62, bottom 34, and `nav 759→812`. A pre-zoom-lock pinch-out screenshot shows the **same N3X nav at 787→840** while the header and first card stay put: the shell's `--app-h` tracks a temporarily taller visual viewport. Keeping `--app-h: 840px` when the viewport returns to 812 clips the tab labels; it cannot freeze that gesture's visible area. The owner reports AnimeBuddy on the same phone has its bar lower. The `black-translucent` status-bar trial did not change N3X. Current experiment: drop `viewport-fit=cover` and use the opaque `black` status bar so WebKit automatically insets the page above the home indicator. This has **not** been measured on the installed phone. Pinch zoom remains blocked; touch-landscape shows a rotate-back prompt, not a hardware orientation lock.
...
**The old dead band disappeared after the owner deleted and re-added the Home Screen icon.** The refreshed app reaches the screen bottom, but the new screenshot shows the tab labels overlapping the gesture indicator. The shell now leaves the device inset intact and reserves a 34px minimum on installed iOS phones. That updated placement still needs confirmation on the actual device; Chromium cannot emulate iOS Home Screen insets.
...
1. **The dead band under the nav** — the original icon's viewport ended at 812; reinstalling the icon resolved that part. The subsequent home-indicator overlap prompted a separate safe-area fix; device confirmation is pending.
