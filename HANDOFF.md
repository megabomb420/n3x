...
Stack: TanStack Start, React 19, Tailwind v4, Vite, Nitro preset `vercel`. App shell is a locked full-height column: `html, body { height: 100% }` and the shell is `h-full`, so header and nav do not scroll, only `<main>` does. Do not use `100svh`/`100dvh` there and do not switch to `position: fixed` or document scroll — on iOS 26 standalone (`viewport-fit=cover`) `100svh` resolves to the screen *minus* the safe-area insets, so the column stops ~96 pt short and leaves a dead band under the nav. `viewport-fit=cover` is required: `.safe-top`/`.safe-bottom` pad with `env(safe-area-inset-*)` (plus 0.75 rem at the top so the header clears iOS 26's top edge blur), and they only apply in `display-mode: standalone` so the Grok/Safari webviews, which already sit below the notch, are not double-padded.
